...
function indexedTree(tensors) {
  return createTensorMetadataIndex(tensors).tensorNameTree;
}
...
test('builds cumulative tensor-name prefixes from left to right', () => {
  ...
  const root = indexedTree(tensors);
...
test('keeps leaf metadata without assigning architecture semantics', () => {
  ...
  const root = indexedTree([tensor, expertTensor]);
  ...
  assert.deepEqual(
    { name: weight.tensors[0].name, shape: weight.tensors[0].shape, dtype: weight.tensors[0].dtype },
    { name: tensor.name, shape: tensor.shape, dtype: tensor.dtype },
  );
  assert.deepEqual(
    { name: expertWeight.tensors[0].name, shape: expertWeight.tensors[0].shape, dtype: expertWeight.tensors[0].dtype },
    { name: expertTensor.name, shape: expertTensor.shape, dtype: expertTensor.dtype },
  );
  assert.deepEqual(weight.children, []);
  assert.deepEqual(Object.keys(weight).filter((key) => !['members', 'repeatCount', 'repeatIds', 'stats'].includes(key)).sort(), [
    'children',
    'directChildCount',
    'numeric',
    'prefix',
...
test('builds Repeated Tensor Groups from matching Numeric Path Branches', () => {
  ...
  const grouped = indexedTree(tensors);
...
test('Tensor Metadata Index owns weight and subtree accounting without mutating inputs', () => {
  const tensors = [
    { name: 'model.layers.0.weight', shape: [2, 3], dtype: 'BF16' },
    { name: 'model.layers.1.weight', shape: [2, 3], dtype: 'F32' },
  ];
  const index = createTensorMetadataIndex(tensors);
  const layers = descendant(index.tensorNameTree, 'model', 'layers');

  assert.equal('params' in tensors[0], false);
  assert.equal(index.summary.totalParams, 12);
  assert.equal(index.totalWeightBytes, 36);
  assert.equal(index.tensorNameTree.stats.bytes, 36);
  assert.equal(layers.stats.params, 12);
  assert.deepEqual(index.tensorNamePatterns, [
    {
      key: 'model.layers.*.weight',
      bytes: 36,
      dtypes: ['BF16', 'F32'],
    },
  ]);
